...
// DEEP SLEEP / LIGHT SLEEP RETENTION VARIABLES
RTC_DATA_ATTR int bootCount = 0;
RTC_DATA_ATTR int prevStatus = -1;
RTC_DATA_ATTR bool timeSynced = false;   
RTC_DATA_ATTR bool distressMode = false;
RTC_DATA_ATTR bool wokeFromLightSleep = false; 
RTC_DATA_ATTR long retainedTime = 0; // <-- ADDED FOR CLOCK RESTORATION
...
// The portal's interval is the desired time between sample-cycle starts, not
// extra sleep added after sampling. Keep a short sleep even if processing runs
// longer than a very small configured interval.
const uint64_t MIN_TIMER_SLEEP_US = 100000ULL;
// Some rechargeable power banks shut off when ESP32 light-sleep current falls
// below their minimum-load threshold. Keep the CPU awake during the interval
// for power-bank compatibility. Set false for a directly connected LiPo.
const bool KEEP_POWER_BANK_AWAKE = true;

uint64_t timerSleepForIntervalUs(int intervalSeconds, unsigned long activeMs) {
  uint64_t intervalUs = (uint64_t)max(intervalSeconds, 1) * 1000000ULL;
  uint64_t activeUs = (uint64_t)activeMs * 1000ULL;
  return (activeUs + MIN_TIMER_SLEEP_US < intervalUs)
      ? intervalUs - activeUs
      : MIN_TIMER_SLEEP_US;
}
...
bool gyro_signals() {
  if (!sensorAvailable) {
    instAccelXYZ[0] = instAccelXYZ[1] = instAccelXYZ[2] = 0.0;
    return false;
  }
  ...
  if (Wire.endTransmission(false) != 0 ||
      Wire.requestFrom((uint16_t)sensorAddress, (uint8_t)6, true) != 6) {
...
void gyro_init() {
  ...
  if (!sensorAvailable) {
    Serial.println("[SENSOR ERROR] No accelerometer found at 0x68 or 0x69; check power, SDA and SCL.");
    return;
  }
...
void handleRoot() {
  ...
  html += "<label>Sampling Interval (Sec, start-to-start):</label><br><input type=\"number\" min=\"1\" name=\"sleep\" value=\"" + String(sleepSeconds) + "\"><br>";
...
void setup() {
  ...
  // MODIFIED WAKE LOGIC WITH CLOCK RESTORATION
  esp_sleep_wakeup_cause_t wakeReason;
  if (wokeFromLightSleep) {
      wakeReason = ESP_SLEEP_WAKEUP_TIMER;
      wokeFromLightSleep = false;
      
      // RESTORE THE CLOCK TO PREVENT WIFI PANIC
      struct timeval tv;
      tv.tv_sec = retainedTime;
      tv.tv_usec = 0;
      settimeofday(&tv, NULL);
  } else {
  ...
  uint64_t scheduledTimerSleepUs = 0;
  if (isRunning && timeSynced) {
    unsigned long cycleStartMs = millis();
    unsigned long bootToCycleMs = cycleStartMs - setupStartMs;
    bootCount++;
    Serial.printf("Boot Count: %d\n", bootCount);
    Serial.printf("[TIMING] cycle_start_ms=%lu boot_to_cycle_ms=%lu target_interval_ms=%lu\n",
                  cycleStartMs, bootToCycleMs, (unsigned long)sleepSeconds * 1000UL);
    blinkLED(2, 200); 
    ...
    int bootsPerMaxLog = (maxLoggedIntervalMins * 60) / sleepSeconds;
    ...
    // The next boot has nearly the same setup overhead as this boot. Include it
    // in the budget so the visible double-flash interval remains on target.
    // A portal can stay open for minutes; that one-time delay is not reboot
    // overhead and must not be subtracted from future sampling cycles.
    unsigned long estimatedNextBootMs = (bootToCycleMs <= 5000UL) ? bootToCycleMs : 0UL;
    scheduledTimerSleepUs = timerSleepForIntervalUs(sleepSeconds, activeMs + estimatedNextBootMs);
    unsigned long sleepMs = (unsigned long)(scheduledTimerSleepUs / 1000ULL);
    Serial.printf("[TIMING] active_ms=%lu sleep_ms=%lu estimated_next_boot_ms=%lu predicted_flash_interval_ms=%lu\n",
                  activeMs, sleepMs, estimatedNextBootMs, activeMs + sleepMs + estimatedNextBootMs);
    Serial.flush();
    esp_sleep_enable_timer_wakeup(scheduledTimerSleepUs);
  } else {
    if (distressMode) {
      digitalWrite(LED_PIN, LOW);
      delay(30); digitalWrite(LED_PIN, HIGH); delay(100);
      digitalWrite(LED_PIN, LOW); delay(30); digitalWrite(LED_PIN, HIGH);
      Serial.println("Distress Mode Active. Sleeping for 4 seconds...");
      scheduledTimerSleepUs = 4000000ULL;
      esp_sleep_enable_timer_wakeup(scheduledTimerSleepUs);
    } else {
  ...
  // MODIFIED SLEEP LOGIC WITH TIME SAVING
  time_t now;
  time(&now);
  // Account only for the actual timer sleep. Awake processing time has already
  // advanced the system clock.
  if (scheduledTimerSleepUs > 0) {
    // The clock has one-second resolution here, so round the actual timer sleep.
    retainedTime = now + (scheduledTimerSleepUs + 500000ULL) / 1000000ULL;
  } else {
    retainedTime = now + sleepSeconds;
  }

  wokeFromLightSleep = true;
  if (KEEP_POWER_BANK_AWAKE && isRunning && scheduledTimerSleepUs > 0) {
    unsigned long awakeWaitMs = (unsigned long)(scheduledTimerSleepUs / 1000ULL);
    // Sampling is complete and WiFi is off, so a lower clock is sufficient for
    // the button-aware wait while still drawing a continuous load.
    setCpuFrequencyMhz(80);
    Serial.printf("[POWER] WiFi off; staying awake at %u MHz for %lu ms to keep battery pack enabled.\n",
                  getCpuFrequencyMhz(), awakeWaitMs);
    Serial.flush();
    smartDelay(awakeWaitMs);
  } else {
